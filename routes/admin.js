...
const {
  ...
  MessGroup, MessMember, MessMenu, MessAttendance, MessExpense, MessTask, MessContribution,
} = require('../models');

const FEATURE_KEYS = ['loans', 'rent', 'bills', 'expenses', 'savings', 'committees', 'investments', 'salary', 'mess'];
...
router.delete('/users/:id', async (req, res) => {
  try {
    ...
    // Mess groups this user OWNS are torn down completely — everything
    // other members logged in them goes with it, same as deleting a
    // committee or a bill this user owns. Groups they merely joined are
    // left intact for the other members; we just drop this user's own
    // membership + anything they personally logged in those groups stays
    // (paid_by/created_by just points at a since-deleted account) so the
    // group's shared history isn't silently rewritten.
    const ownedMessGroups = await MessGroup.find({ owner: targetId }).select('_id');
    const ownedMessGroupIds = ownedMessGroups.map(g => g._id);
    await Promise.all([
      MessMember.deleteMany({ group: { $in: ownedMessGroupIds } }),
      MessMenu.deleteMany({ group: { $in: ownedMessGroupIds } }),
      MessAttendance.deleteMany({ group: { $in: ownedMessGroupIds } }),
      MessExpense.deleteMany({ group: { $in: ownedMessGroupIds } }),
      MessTask.deleteMany({ group: { $in: ownedMessGroupIds } }),
      MessContribution.deleteMany({ group: { $in: ownedMessGroupIds } }),
      MessGroup.deleteMany({ _id: { $in: ownedMessGroupIds } }),
      MessMember.deleteMany({ user: targetId, group: { $nin: ownedMessGroupIds } }),
    ]);
